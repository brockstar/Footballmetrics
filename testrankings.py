...
class TestRankings(unittest.TestCase):
    def setUp(self):
        ...
        if not os.path.isfile(self.db_path):
            raise IOError('Database test.db not found!')

    def test_fisb_rating(self):
        fisb = fm_rkg.FISB_Ranking(None, None, self.db_path)
        ratings = fisb.calculate_ranking()
    ...
    def test_fisb_bootstrap(self):
        fisb = fm_rkg.FISB_Ranking(None, None, self.db_path)
        ratings = fisb.calculate_ranking(10000)
        result = {'A': 2.44, 'B': -4.40, 'C': -5.35, 'D': 7.32, 'Home field advantage': -0.25}
        for k in ratings:
            self.assertAlmostEqual(ratings[k], result[k], places=0)

    def test_fisb_db_not_found(self):
        self.assertRaises(IOError, fm_rkg.FISB_Ranking, None, None, 'no.db')
    ...
    def test_ml_rating(self):
        ml = fm_rkg.ML_Ranking(None, None, self.db_path)
    ...
    def test_srs_rating(self):
        s = fm_rkg.SRS(None, None, self.db_path)
